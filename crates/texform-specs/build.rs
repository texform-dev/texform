...
mod command_symbol_facades {
    include!("codegen/command_symbol_facades.rs");
}
...
fn main() {
    ...
    println!(
        ...
        command_symbol_facades_path.display()
    );

    let command_symbol_facades =
        load_command_symbol_facades(command_symbol_facades::COMMAND_SYMBOL_FACADES);
    ...
    fs::write(
        out_path,
        generate_builtin_source(packages.as_slice(), &command_symbol_facades),
    )
...
fn load_command_symbol_facades(entries: &[(&str, &str)]) -> HashMap<String, String> {
    let mut by_name = HashMap::new();
    let mut facades = HashSet::new();

    for &(name, facade) in entries {
        if name.chars().all(|ch| ch.is_ascii_alphanumeric()) {
            panic!("command symbol facade entry {name:?} is not a control-symbol name");
        }
        if !is_valid_symbol_facade_ident(facade) {
            panic!(
                "command symbol facade for {name:?} must be a valid uppercase Rust identifier with a leading underscore, got {facade:?}"
            );
        }
        if !facades.insert(facade.to_string()) {
            panic!("duplicate command symbol facade `{facade}`");
        }
        if by_name
            .insert(name.to_string(), facade.to_string())
            .is_some()
        {
            panic!("duplicate command symbol facade entry for {name:?}");
        }
    }
...
fn is_valid_symbol_facade_ident(facade: &str) -> bool {
    let mut chars = facade.chars();
    ...
    for ch in chars {
        has_body = true;
        if !(ch == '_' || ch.is_ascii_uppercase() || ch.is_ascii_digit()) {
            return false;
        }
    }
...
fn generate_builtin_source(
    packages: &[BuiltinPackageSource],
    command_symbol_facades: &HashMap<String, String>,
) -> String {
    let mut out = String::new();
    out.push_str("// @generated by build.rs\n");

    for package in packages {
        emit_package_module(&mut out, package, command_symbol_facades);
    }
...
fn emit_package_module(
    ...
    emit_command_module(out, package.commands.as_slice(), command_symbol_facades);
...
fn emit_command_module(
    out: &mut String,
    records: &[CommandRecordSource],
    command_symbol_facades: &HashMap<String, String>,
) {
    out.push_str("    pub mod cmd {\n");
    out.push_str("        use super::*;\n");
    let facades = resolve_command_facade_idents(
        records
            .iter()
            .map(|record| record.name.as_str())
            .collect::<Vec<_>>()
            .as_slice(),
        command_symbol_facades,
    );
...
fn command_facade_ident(
    name: &str,
    command_symbol_facades: &HashMap<String, String>,
) -> Option<String> {
    if name.chars().all(|ch| ch.is_ascii_alphanumeric()) {
        return facade_ident(name);
    }

    command_symbol_facades.get(name).cloned()
}

fn resolve_command_facade_idents(
    names: &[&str],
    command_symbol_facades: &HashMap<String, String>,
) -> Vec<String> {
    let base = names
        .iter()
        .map(|name| command_facade_ident(name, command_symbol_facades))
        .collect::<Vec<_>>();
    let facades = dedupe_facade_idents(base);
    let missing = names
        .iter()
        .zip(facades.iter())
        .filter_map(|(name, facade)| facade.is_none().then_some(*name))
        .collect::<Vec<_>>();

    if !missing.is_empty() {
        panic!(
            "command names must have stable builtin facade identifiers; add symbol mappings for: {}",
            missing
                .iter()
    ...
    facades.into_iter().map(Option::unwrap).collect()
}
...
fn resolve_facade_idents(names: &[&str]) -> Vec<Option<String>> {
    let base = names
        ...
        .collect::<Vec<_>>();
    dedupe_facade_idents(base)
}

fn dedupe_facade_idents(base: Vec<Option<String>>) -> Vec<Option<String>> {
