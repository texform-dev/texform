...
- Add formula and knowledge subcommands to `texform`
- Print syntax trees and token lists for human reading
- Add `texform-cli` with the `serve` normalizer protocol

### Changed

- Cache rewrite trigger indexes in `Plan`

### Fixed

- Stabilize normalization across phase boundaries
- **Breaking:** Separate prime symbols from quote superscripts
- Preserve lexical command boundaries
- Preserve local attribute scopes
